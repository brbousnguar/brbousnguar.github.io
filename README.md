...
- **Notes (`/notes/`)** — English-only technical articles built from Markdown by `tools/build_notes.py`, with an Atom feed, TechArticle JSON-LD, a `.md` copy of each note and `llms-full.txt` for AI agents.
...
├── sitemap.xml, robots.txt     # sitemap generated by tools/build_notes.py
...
└── tools/
    ...
    └── build_notes.py          # builds /notes/, the feed, sitemap and llms-full.txt
```
...
- To publish a note: write `notes/<slug>.md`, run `python3 tools/build_notes.py`, commit sources and outputs together.
